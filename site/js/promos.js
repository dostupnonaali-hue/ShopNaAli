/**
 * ShopDoBaksa - Promos Page Dynamic Codes (Ukrainian)
...
(function () {
    ...
    async function init() {
        try {
            ...
            if (promos.length === 0) {
                container.innerHTML = '<p style="color:var(--text-muted);text-align:center;">Промокодів поки немає</p>';
                return;
            }
            ...
            container.innerHTML = promos.map(function(p) {
                ...
                return '<div class="promo-card promo-card--parser">' +
                    ...
                    '</div>' +
                    '<div class="promo-card__code-wrap" onclick="copyCode(\'' + escapeAttr(p.code) + '\')" title="Натисніть, щоб скопіювати">' +
                        '<span class="promo-card__code">' + escapeHtml(p.code) + '</span>' +
                        '<button class="promo-card__copy" aria-label="Копіювати">📋</button>' +
                    '</div>' +
                '</div>';
            }).join('');
            ...
            container.innerHTML = '<p style="color:var(--text-muted);text-align:center;">Не вдалося завантажити промокоди</p>';
        }
    }
